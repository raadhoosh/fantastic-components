...
class ReactCropper extends Component {
  ...
  onComponentKeyDown(e) {
    ...
    if (keyCode === ReactCropper.arrowKey.left) {
      crop.x -= ReactCropper.nudgeStep;
      nudged = true;
    } else if (keyCode === ReactCropper.arrowKey.right) {
      crop.x += ReactCropper.nudgeStep;
      nudged = true;
    } else if (keyCode === ReactCropper.arrowKey.up) {
      crop.y -= ReactCropper.nudgeStep;
      nudged = true;
    } else if (keyCode === ReactCropper.arrowKey.down) {
      crop.y += ReactCropper.nudgeStep;
      nudged = true;
    }
  ...
  resizeCrop() {
    ...
    if (crop.aspect || ReactCropper.xyOrds.indexOf(ord) > -1) {
    ...
    } else if (ReactCropper.xOrds.indexOf(ord) > -1) {
      crop.x = clamp(newX, 0, 100 - newSize.width);
      crop.width = newSize.width;
    } else if (ReactCropper.yOrds.indexOf(ord) > -1) {
      crop.y = clamp(newY, 0, 100 - newSize.height);
      crop.height = newSize.height;
    }
  ...
  nextCropState(crop) {
    const nextCrop = {...ReactCropper.defaultCrop, ...crop};
    this.cropInvalid = isCropInvalid(nextCrop);
    return nextCrop;
  }
